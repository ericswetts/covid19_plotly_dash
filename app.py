...
#Load reference CSV
country_reference = pd.read_csv('./geodata/country_reference.csv', index_col = 0)
...
#Access pre-cleaned COVID data
df = pd.read_csv('assets/covid_data.csv')
...
fast_fact_card = dbc.Col(
            dbc.ListGroup(
                dbc.ListGroupItem(children = [
                    dbc.ListGroupItem(dcc.Markdown('''# Fast Facts''')),
                    html.Hr(),
                    dbc.ListGroupItem(f"Global Cases : {total_cases}"),
                    dbc.ListGroupItem(f"Global Deaths : {total_deaths}"),
                    dbc.ListGroupItem(f"Days Since First Reported Case : {pandemic_length}"),
                    dbc.ListGroupItem(f"Countries reporting > 100 cases / Day : {active_countries}")
                ], 
...
intro_text = html.Div([
    html.P([
        ...
        html.Br(), html.Br(),
        'Data for this dashboard is provided by Johns Hopkins University, through the ', open_api_link, '. I assume no responsibility for any incorrect information, do not claim the accuracy of the information presented, and provide this data only for educational purposes.'
    ]),
    html.Br(),
    html.P([
        html.Span('Data current through: ', style={'font-weight': 'bold'}), 
        html.Span(f"{last_updated}", style={'font-weight': 'bold'})
    ])
])
...
def update_country_card(iso3, new_metric, new_group):
    ...
    country_card_list_content = dbc.ListGroup(
            dbc.ListGroupItem(
                children = [
                    dbc.ListGroupItem(html.H4('{} Summary'.format(new_country))),
                    html.Hr(),
#                     dbc.ListGroupItemHeading(country_card_text),
#                     html.Hr(),
                    dbc.ListGroupItem("Statistics"),
                    dbc.ListGroupItem(f"Total Cases : {int(country_cases):n}"),
                    dbc.ListGroupItem(f"Cases per 1M : {int(country_cases_per_1m):n}"),
                    dbc.ListGroupItem(f"Total Deaths : {int(country_deaths):n}"),
                    dbc.ListGroupItem(f"Deaths per 1M : {int(country_deaths_per_1m):n}"),
                    dbc.ListGroupItem(f"Life Expectancy (2018) : {country_life_exp} years"),
                    dbc.ListGroupItem(f"Press Freedom Index Rank (2020) : {int(wfpi_rank)} out of {int(wfpi_max_rank)}"),
                    dbc.ListGroupItem(f"United Nations Regional Group : {un_group}"), 
                    dbc.ListGroupItem(f"World Bank ATLAS Group : {income_group}")
